...
params.bed2interval_description = "only convert chrosome 4 to intevallist"
params.bed2interval_chr = "4"
params.bed2interval_script = "./sak_data/test.sh"
params.bed2interval_dockerimg = "broadinstitute/gatk:4.2.2.0"
params.bed2interval_argument = "cat !{bed} | awk -v var=!{chr} '{if(\$1 == var) print}' > temp.bed; val_var=!{chr}; bash !{script} temp.bed human_g1k_v37_decoy.dict"
...
params.addchr_argument = "file=\$(ls *.interval_list); cat \$file | sed 's/^/chr/' > \${file%.*}.chr.interval_list; echo only generate interval_list for chr !{bed2interval_val_var[0]}"
...
workflow {
    ...
    */
    ...
    if (params.bed2interval_dockerimg) {
        BED2INTERVALDOC(BED2INTERVAL_bed,BED2INTERVAL_dict,  BED2INTERVAL_ScriptFiles, params.bed2interval_argument, params.bed2interval_dockerimg, params.bed2interval_sakcpu, params.bed2interval_sakmem, params.bed2interval_saktime, params.bed2interval_name, params.bed2interval_description, params.bed2interval_chr, params.bed2interval_outputDir)
    } else {
        BED2INTERVAL(BED2INTERVAL_bed,BED2INTERVAL_dict,  BED2INTERVAL_ScriptFiles, params.bed2interval_argument, params.bed2interval_sakcpu, params.bed2interval_sakmem, params.bed2interval_saktime, params.bed2interval_name, params.bed2interval_description, params.bed2interval_chr, params.bed2interval_outputDir)   
    }
    //* ## step cmd example SAK
    ...
    if (params.addchr_dockerimg) {
        ADDCHRDOC(ADDCHR_file, BED2INTERVALDOC.out.file.collect(), BED2INTERVALDOC.out.val_var.collect(), ADDCHR_ScriptFiles, params.addchr_argument, params.addchr_dockerimg, params.addchr_sakcpu, params.addchr_sakmem, params.addchr_saktime, params.addchr_name, params.addchr_outputDir)
    } else {
        ADDCHR(ADDCHR_file, BED2INTERVALDOC.out.file.collect(), BED2INTERVALDOC.out.val_var.collect(), ADDCHR_ScriptFiles, params.addchr_argument, params.addchr_sakcpu, params.addchr_sakmem, params.addchr_saktime, params.addchr_name, params.addchr_outputDir)   
    }
