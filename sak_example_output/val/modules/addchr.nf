...
val outputDir
    ...
    echo "upstream files check: !{bed2interval_file}, !{bed2interval_val_var}"
...
file=\$(ls *.interval_list)
 cat \$file | sed 's/^/chr/' > \${file%.*}.chr.interval_list
 echo only generate interval_list for chr !{bed2interval_val_var[0]} 2>&1 | tee -a sak-nf_\$(date +%Y%m%d_%H%M%S).log 
