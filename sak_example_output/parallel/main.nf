...
params.bed2interval_argument = "name=!{bed[0]}; echo \$name; cat !{bed} > \${name%%.*}.bed; bash !{script} \${name%%.*}.bed human_g1k_v37_decoy.dict"
...
params.bed2interval_input_bed = "./sak_data/*{small,large}.bed"
...
workflow {
    ...
    */
    ...
    if(params.bed2interval_input_bed) {BED2INTERVAL_bed = Channel.fromPath(params.bed2interval_input_bed).buffer(size : 2)} else {BED2INTERVAL_bed = Channel.fromPath("$baseDir" + "/data/input_sanitychk")}
    //* ## step cmd example SAK
