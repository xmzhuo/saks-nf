...
params.scatter_name = "scatter"
params.scatter_script = ""
params.scatter_dockerimg = ""
params.scatter_argument = "for chr in \$(cat !{file} | awk '{print \$1}' | sort | uniq); do echo \$chr; cat !{file} | awk -v var=\$chr '{if(\$1 == var) print}' > \$chr.bed; done"
params.scatter_outputDir = "./results/scatter"
params.scatter_sakcpu = "1"
params.scatter_sakmem = "1.GB"
params.scatter_saktime = "1.hour"
params.scatter_input_file = "./sak_data/test.bed"
...
params.bed2interval_argument = "bash !{script} !{scatter_file} human_g1k_v37_decoy.dict"
...
params.addchr_argument = "for file in !{bed2interval_file}; do cat \$file |  awk '{if(\$1 !~ \"@\") \$1=\"chr\"\$1; print}' > \${file%.*}.chr.interval_list; done"
...
include { SCATTER } from './modules/scatter'
include { SCATTERDOC } from './modules/scatter_docker'
...
workflow {
    ...
    */
    ...
    if(params.scatter_input_file) {SCATTER_file = Channel.fromPath(params.scatter_input_file).toSortedList()} else {SCATTER_file = Channel.fromPath("$baseDir" + "/data/input_sanitychk")}
    if(params.scatter_script) {SCATTER_ScriptFiles = Channel.fromPath(params.scatter_script).toSortedList()} else {SCATTER_ScriptFiles = Channel.fromPath("$baseDir" + "/bin/script_sanitychk")}
    
    if (params.scatter_dockerimg) {
        SCATTERDOC(SCATTER_file,  SCATTER_ScriptFiles, params.scatter_argument, params.scatter_dockerimg, params.scatter_sakcpu, params.scatter_sakmem, params.scatter_saktime, params.scatter_outputDir)
    } else {
        SCATTER(SCATTER_file,  SCATTER_ScriptFiles, params.scatter_argument, params.scatter_sakcpu, params.scatter_sakmem, params.scatter_saktime, params.scatter_outputDir)   
    }
    //* ## step cmd example SAK
    ...
    if (params.bed2interval_dockerimg) {
        BED2INTERVALDOC(BED2INTERVAL_dict, SCATTER.out.file.collect().sort().flatten().buffer(size:1), BED2INTERVAL_ScriptFiles, params.bed2interval_argument, params.bed2interval_dockerimg, params.bed2interval_sakcpu, params.bed2interval_sakmem, params.bed2interval_saktime, params.bed2interval_outputDir)
    } else {
        BED2INTERVAL(BED2INTERVAL_dict, SCATTER.out.file.collect().sort().flatten().buffer(size:1), BED2INTERVAL_ScriptFiles, params.bed2interval_argument, params.bed2interval_sakcpu, params.bed2interval_sakmem, params.bed2interval_saktime, params.bed2interval_outputDir)   
    }
    //* ## step cmd example SAK
