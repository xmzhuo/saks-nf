...
params.bed2interval_input = "./sak_data/*.{bed,dict}"
...
workflow {
    ...
    */
    ...
    //* ## step cmd example BED2INTERVAL
    if(params.bed2interval_input) {BED2INTERVAL_InFiles = Channel.fromPath(params.bed2interval_input).toSortedList()} else {BED2INTERVAL_InFiles = Channel.fromPath(params.defdir + "/data/input_sanitychk")}
    BED2INTERVAL_UpStream = Channel.fromPath(params.defdir + "/data/upstream_sanitychk")
    BED2INTERVAL_UpStream.view()
    if(params.bed2interval_script) {BED2INTERVAL_ScriptFiles = Channel.fromPath(params.bed2interval_script).toSortedList()} else {BED2INTERVAL_ScriptFiles = Channel.fromPath(params.defdir + "/bin/script_sanitychk")}
    
    if (params.bed2interval_dockerimg) {
        BED2INTERVALDOC(BED2INTERVAL_InFiles, BED2INTERVAL_UpStream, BED2INTERVAL_ScriptFiles, params.bed2interval_argument, params.bed2interval_dockerimg, params.bed2interval_sakcpu, params.bed2interval_sakmem, params.bed2interval_saktime, params.bed2interval_outputDir)
    } else {
        BED2INTERVAL(BED2INTERVAL_InFiles, BED2INTERVAL_UpStream, BED2INTERVAL_ScriptFiles, params.bed2interval_argument, params.bed2interval_sakcpu, params.bed2interval_sakmem, params.bed2interval_saktime, params.bed2interval_outputDir)   
    }
    //* ## step cmd example ADDCHR
    if(params.addchr_input) {ADDCHR_InFiles = Channel.fromPath(params.addchr_input).toSortedList()} else {ADDCHR_InFiles = Channel.fromPath(params.defdir + "/data/input_sanitychk")}
    ADDCHR_UpStream = Channel.fromPath(params.defdir + "/data/upstream_sanitychk").concat(BED2INTERVALDOC.out)|collect
    ADDCHR_UpStream.view()
    if(params.addchr_script) {ADDCHR_ScriptFiles = Channel.fromPath(params.addchr_script).toSortedList()} else {ADDCHR_ScriptFiles = Channel.fromPath(params.defdir + "/bin/script_sanitychk")}
    
    if (params.addchr_dockerimg) {
        ADDCHRDOC(ADDCHR_InFiles, ADDCHR_UpStream, ADDCHR_ScriptFiles, params.addchr_argument, params.addchr_dockerimg, params.addchr_sakcpu, params.addchr_sakmem, params.addchr_saktime, params.addchr_outputDir)
    } else {
        ADDCHR(ADDCHR_InFiles, ADDCHR_UpStream, ADDCHR_ScriptFiles, params.addchr_argument, params.addchr_sakcpu, params.addchr_sakmem, params.addchr_saktime, params.addchr_outputDir)   
    }
